...
@Repository
public interface CaseStatusWiseReportRepository extends JpaRepository<LitigationCaseInfo, Long> {

    @Query(value = "SELECT CS.NAME AS caseStatus, " +
            ...
            "FROM CASE_STATUS CS " +
            "LEFT JOIN LITIGATION_CASE_INFO CBD ON CBD.STATUS_ID = CS.ID AND UPPER(CBD.BUSINESS_SEGMENT) = 'CBD' AND CBD.CASE_TYPE_ID IN (SELECT ID FROM CASE_TYPE WHERE NAME = ?1) " +
            "LEFT JOIN LITIGATION_CASE_INFO SME ON SME.STATUS_ID = CS.ID AND UPPER(SME.BUSINESS_SEGMENT) = 'SME' AND SME.CASE_TYPE_ID IN (SELECT ID FROM CASE_TYPE WHERE NAME = ?1) " +
            "LEFT JOIN LITIGATION_CASE_INFO RBD ON RBD.STATUS_ID = CS.ID AND UPPER(RBD.BUSINESS_SEGMENT) = 'RBD' AND RBD.CASE_TYPE_ID IN (SELECT ID FROM CASE_TYPE WHERE NAME = ?1) " +
            "LEFT JOIN LITIGATION_CASE_INFO CARD ON CARD.STATUS_ID = CS.ID AND UPPER(CARD.BUSINESS_SEGMENT) = 'CARD' AND CARD.CASE_TYPE_ID IN (SELECT ID FROM CASE_TYPE WHERE NAME = ?1) " +
            "GROUP BY CS.NAME " +
            "ORDER BY CS.NAME", nativeQuery = true)
