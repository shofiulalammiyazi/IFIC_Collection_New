...
public interface AccountInformationRepository extends JpaRepository<AccountInformationEntity, Long> {
    ...
    @Query(value = "SELECT * FROM ACCOUNT_INFORMATION_ENTITY WHERE LOANACNO like %?1% AND IS_ESCALATED = 'Y' AND IS_CLOSED = 'N'", nativeQuery = true)
    ...
    @Query(value = "SELECT AIE.* " +
            ...
            "   AND TO_CHAR(AIE.SMS_SENT_DATE+3,'YYYY-MM-DD') = TO_CHAR(SYSDATE,'YYYY-MM-DD')" +
            " AND TO_NUMBER(OVERDUE)>0", nativeQuery = true)
