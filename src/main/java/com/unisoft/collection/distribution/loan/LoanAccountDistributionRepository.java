...
//    List<AllocationDetails> findByCurrentMonthAllocationDistribution(Date startDate, Date endDate);
    ...
    @Query(value = "SELECT * FROM LOAN_ACCOUNT_DISTRIBUTION_INFO WHERE TO_CHAR(CREATED_DATE, 'dd-mm-yyyy') between :startDate AND :endDate " +
            "AND LATEST = '1' order by CREATED_DATE desc ", nativeQuery = true)
    ...
    @Query(value ="SELECT AIE.CUSTOMER_NAME, " +
            "       AIE.LOANACNO, " +
            "       AIE.MOBILE, " +
            "       AIE.BRANCH_MNEMONIC, " +
            ...
            "       LEFT JOIN EMPLOYEE_INFO_ENTITY E on PAL.TEAM_LEAD_ID = E.ID" +
            " WHERE LADI.LATEST='1'",nativeQuery = true)
